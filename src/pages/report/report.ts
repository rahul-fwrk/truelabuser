...
export class ReportPage {
			...
			if (response.error == 0) {
				...
				if (response.data[0].PatientTest.report != null) {
					...
					if (this.test_name == 'PT/INR') {
						...
						var flagsplit = this.values.PatientTest.flag
						
						var str = flagsplit.toUpperCase();
						console.log('Uppper case->',str)
						var flags = str.split("/");
						this.inrflag = flags[0];
						console.log(this.inrflag);
						this.ptflag = flags[1];
						...
						var str = this.values.PatientTest.flag.toUpperCase();
	...
	reportSub(signup) {
		console.log(signup.value.remarks);
		//  this.Loader.present();
		this.test_id = this.navParams.get('test_id');
		var url: string = this.appsetting.base_url + 'patients/patientreport';

		console.log('pt/inr : '+ signup.value.report)
		...
		if(signup.value.remarks == undefined){
			signup.value.remarks = '';
			console.log('signup.value.remarks ', signup.value.remarks )
		} else if(signup.value.remarks == null){
			signup.value.remarks = '';
			console.log('signup.value.remarks ', signup.value.remarks )
		}
		var postdata = {
